...
A dedicated **Statutory Documents** section (separate from per-invoice
attachments) stores compliance PDFs the bond warehouse operation is required
to keep on file — Bonded Warehouse License, CHA License, IEC, GST
Registration, PAN, AEO Certificate, Fire/Pollution NOCs, Insurance Policies,
Statutory Audit Reports, etc.

- **PDF-only**: uploads are rejected unless the file is a `.pdf`.
- **Expiry tracking**: each document shows a computed status — Active,
  Expiring Soon (within 30 days), or Expired — with dashboard KPI cards and
  list filters for each. Documents without an expiry (e.g. PAN, Incorporation
  Certificate) are simply always Active.
- Accessible from the sidebar (**Statutory Documents**); upload/edit is
  restricted to Administrator, Bond Superintendent, and Accounts Officer
  roles; delete is Administrator-only.
